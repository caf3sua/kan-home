...
public class CountryResource {
    ...
    @Timed
    public ResponseEntity<List<CountryDTO>> getAllCountriesOnMap() {
        log.debug("REST request to get a page of Countries");
        List<CountryDTO> lst = countryService.findAll();
        return new ResponseEntity<>(lst, null, HttpStatus.OK);
    }
    
    @GetMapping("/countries-by-code/{countryCode}")
    @Timed
    public ResponseEntity<CountryDTO> getCountryByCode(@PathVariable Long countryCode) {
        log.debug("REST request to getCountryByCode : {}", countryCode);
        CountryDTO countryDTO = countryService.findOneByCountryCode(countryCode);
        return ResponseUtil.wrapOrNotFound(Optional.ofNullable(countryDTO));
    }
}
